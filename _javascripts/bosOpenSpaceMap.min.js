 ...
 */
(function(root,factory){if(typeof define==="function"&&define.amd){define(["leaflet"],function(L){return(root.L.MarkerClusterGroup.LayerSupport=factory(L))})}else{if(typeof module==="object"&&module.exports){module.exports=factory(require("leaflet"))}else{root.L.MarkerClusterGroup.LayerSupport=factory(root.L)}}}(this,function(L,undefined){var LMCG=L.MarkerClusterGroup,LMCGproto=LMCG.prototype,EVENTS=L.FeatureGroup.EVENTS;var MarkerClusterGroupLayerSupport=LMCG.extend({statics:{version:"0.1.0"},options:{singleAddRemoveBufferDuration:100},initialize:function(options){LMCGproto.initialize.call(this,options);this._featureGroup=new _ByPassingFeatureGroup();this._featureGroup.on(EVENTS,this._propagateEvent,this);this._nonPointGroup=new _ByPassingFeatureGroup();this._nonPointGroup.on(EVENTS,this._propagateEvent,this);this._layers={};this._proxyLayerGroups={};this._proxyLayerGroupsNeedRemoving={};this._singleAddRemoveBuffer=[]},checkIn:function(layers){var layersArray=this._toArray(layers);this._checkInGetSeparated(layersArray);return this},checkOut:function(layers){var layersArray=this._toArray(layers),separated=this._separateSingleFromGroupLayers(layersArray,{groups:[],singles:[]}),groups=separated.groups,singles=separated.singles,i,layer;for(i=0;i<singles.length;i++){layer=singles[i];delete this._layers[L.stamp(layer)];delete layer._mcgLayerSupportGroup}this._originalRemoveLayers(singles);for(i=0;i<groups.length;i++){layer=groups[i];this._dismissProxyLayerGroup(layer)}return this},addLayers:function(layers){var layersArray=this._toArray(layers),separated=this._checkInGetSeparated(layersArray),groups=separated.groups,i,group,id;this._originalAddLayers(separated.singles);for(i=0;i<groups.length;i++){group=groups[i];id=L.stamp(group);this._proxyLayerGroups[id]=group;delete this._proxyLayerGroupsNeedRemoving[id];if(this._map){this._map._originalAddLayer(group)}}},addLayer:function(layer){this._bufferSingleAddRemove(layer,"addLayers");return this},_originalAddLayer:LMCGproto.addLayer,_originalAddLayers:LMCGproto.addLayers,removeLayers:function(layers){var layersArray=this._toArray(layers),separated=this._separateSingleFromGroupLayers(layersArray,{groups:[],singles:[]}),groups=separated.groups,singles=separated.singles,i=0,group,id;this._originalRemoveLayers(singles);for(;i<groups.length;i++){group=groups[i];id=L.stamp(group);delete this._proxyLayerGroups[id];if(this._map){this._map._originalRemoveLayer(group)}else{this._proxyLayerGroupsNeedRemoving[id]=group}}return this},removeLayer:function(layer){this._bufferSingleAddRemove(layer,"removeLayers");return this},_originalRemoveLayer:LMCGproto.removeLayer,_originalRemoveLayers:LMCGproto.removeLayers,onAdd:function(map){map._originalAddLayer=map._originalAddLayer||map.addLayer;map._originalRemoveLayer=map._originalRemoveLayer||map.removeLayer;L.extend(map,_layerSwitchMap);var toBeReAdded=this._removePreAddedLayers(map),id,group,i;LMCGproto.onAdd.call(this,map);for(id in this._proxyLayerGroups){group=this._proxyLayerGroups[id];map._originalAddLayer(group)}for(id in this._proxyLayerGroupsNeedRemoving){group=this._proxyLayerGroupsNeedRemoving[id];map._originalRemoveLayer(group);delete this._proxyLayerGroupsNeedRemoving[id]}for(i=0;i<toBeReAdded.length;i++){map.addLayer(toBeReAdded[i])}},_bufferSingleAddRemove:function(layer,operationType){var duration=this.options.singleAddRemoveBufferDuration,fn;if(duration>0){this._singleAddRemoveBuffer.push({type:operationType,layer:layer});if(!this._singleAddRemoveBufferTimeout){fn=L.bind(this._processSingleAddRemoveBuffer,this);this._singleAddRemoveBufferTimeout=setTimeout(fn,duration)}}else{this[operationType](layer)}},_processSingleAddRemoveBuffer:function(){var singleAddRemoveBuffer=this._singleAddRemoveBuffer,i=0,layersBuffer=[],currentOperation,currentOperationType;for(;i<singleAddRemoveBuffer.length;i++){currentOperation=singleAddRemoveBuffer[i];if(!currentOperationType){currentOperationType=currentOperation.type}if(currentOperation.type===currentOperationType){layersBuffer.push(currentOperation.layer)}else{this[currentOperationType](layersBuffer);layersBuffer=[currentOperation.layer]}}this[currentOperationType](layersBuffer);singleAddRemoveBuffer.length=0;clearTimeout(this._singleAddRemoveBufferTimeout);this._singleAddRemoveBufferTimeout=null},_checkInGetSeparated:function(layersArray){var separated=this._separateSingleFromGroupLayers(layersArray,{groups:[],singles:[]}),groups=separated.groups,singles=separated.singles,i,layer;for(i=0;i<groups.length;i++){layer=groups[i];this._recruitLayerGroupAsProxy(layer)}for(i=0;i<singles.length;i++){layer=singles[i];this._removeFromOtherGroupsOrMap(layer);this._layers[L.stamp(layer)]=layer;layer._mcgLayerSupportGroup=this}return separated},_separateSingleFromGroupLayers:function(inputLayers,output){var groups=output.groups,singles=output.singles,isArray=L.Util.isArray,layer;for(var i=0;i<inputLayers.length;i++){layer=inputLayers[i];if(layer instanceof L.LayerGroup){groups.push(layer);this._separateSingleFromGroupLayers(layer.getLayers(),output);continue}else{if(isArray(layer)){this._separateSingleFromGroupLayers(layer,output);continue}}singles.push(layer)}return output},_recruitLayerGroupAsProxy:function(layerGroup){var otherMcgLayerSupportGroup=layerGroup._proxyMcgLayerSupportGroup;if(otherMcgLayerSupportGroup){if(otherMcgLayerSupportGroup===this){return}otherMcgLayerSupportGroup.checkOut(layerGroup)}else{this._removeFromOwnMap(layerGroup)}layerGroup._proxyMcgLayerSupportGroup=this;layerGroup._originalAddLayer=layerGroup._originalAddLayer||layerGroup.addLayer;layerGroup._originalRemoveLayer=layerGroup._originalRemoveLayer||layerGroup.removeLayer;L.extend(layerGroup,_proxyLayerGroup)},_dismissProxyLayerGroup:function(layerGroup){if(layerGroup._proxyMcgLayerSupportGroup===undefined||layerGroup._proxyMcgLayerSupportGroup!==this){return}delete layerGroup._proxyMcgLayerSupportGroup;layerGroup.addLayer=layerGroup._originalAddLayer;layerGroup.removeLayer=layerGroup._originalRemoveLayer;var id=L.stamp(layerGroup);delete this._proxyLayerGroups[id];delete this._proxyLayerGroupsNeedRemoving[id];this._removeFromOwnMap(layerGroup)},_removeFromOtherGroupsOrMap:function(layer){var otherMcgLayerSupportGroup=layer._mcgLayerSupportGroup;if(otherMcgLayerSupportGroup){if(otherMcgLayerSupportGroup===this){return}otherMcgLayerSupportGroup.checkOut(layer)}else{if(layer.__parent){layer.__parent._group.removeLayer(layer)}else{this._removeFromOwnMap(layer)}}},_removeFromOwnMap:function(layer){if(layer._map){layer._map.removeLayer(layer)}},_removePreAddedLayers:function(map){var layers=this._layers,toBeReAdded=[],layer;for(var id in layers){layer=layers[id];if(layer._map){toBeReAdded.push(layer);map._originalRemoveLayer(layer)}}return toBeReAdded},_toArray:function(item){return L.Util.isArray(item)?item:[item]}});var _ByPassingFeatureGroup=L.FeatureGroup.extend({addLayer:function(layer){if(this.hasLayer(layer)){return this}if("on" in layer){layer.on(EVENTS,this._propagateEvent,this)}var id=L.stamp(layer);this._layers[id]=layer;if(this._map){this._map._originalAddLayer(layer)}if(this._popupContent&&layer.bindPopup){layer.bindPopup(this._popupContent,this._popupOptions)}return this.fire("layeradd",{layer:layer})},removeLayer:function(layer){if(!this.hasLayer(layer)){return this}if(layer in this._layers){layer=this._layers[layer]}if("off" in layer){layer.off(EVENTS,this._propagateEvent,this)}var id=L.stamp(layer);if(this._map&&this._layers[id]){this._map._originalRemoveLayer(this._layers[id])}delete this._layers[id];if(this._popupContent){this.invoke("unbindPopup")}return this.fire("layerremove",{layer:layer})},onAdd:function(map){this._map=map;this.eachLayer(map._originalAddLayer,map)},onRemove:function(map){this.eachLayer(map._originalRemoveLayer,map);this._map=null}});var _proxyLayerGroup={addLayer:function(layer){var id=this.getLayerId(layer);this._layers[id]=layer;if(this._map){this._proxyMcgLayerSupportGroup.addLayer(layer)}else{this._proxyMcgLayerSupportGroup.checkIn(layer)}return this},removeLayer:function(layer){var id=layer in this._layers?layer:this.getLayerId(layer);this._proxyMcgLayerSupportGroup.removeLayer(layer);delete this._layers[id];return this}};var _layerSwitchMap={addLayer:function(layer){if(layer._mcgLayerSupportGroup){return layer._mcgLayerSupportGroup._originalAddLayer(layer)}return this._originalAddLayer(layer)},removeLayer:function(layer){if(layer._mcgLayerSupportGroup){return layer._mcgLayerSupportGroup._originalRemoveLayer(layer)}return this._originalRemoveLayer(layer)}};L.markerClusterGroup.layerSupport=function(options){return new L.MarkerClusterGroup.LayerSupport(options)};return MarkerClusterGroupLayerSupport}));
/*!
 * L.Control.Loading is a control that shows a loading indicator when tiles are
 * loading or when map-related AJAX requests are taking place.
 */
(function(){var a=window.console||{error:function(){},warn:function(){}};function b(c){c.Control.Loading=c.Control.extend({options:{delayIndicator:null,position:"topleft",separate:false,zoomControl:null,spinjs:false,spin:{lines:7,length:3,width:3,radius:5,rotate:13,top:"83%"}},initialize:function(d){c.setOptions(this,d);this._dataLoaders={};if(this.options.zoomControl!==null){this.zoomControl=this.options.zoomControl}},onAdd:function(f){if(this.options.spinjs&&(typeof Spinner!=="function")){return a.error("Leaflet.loading cannot load because you didn't load spin.js (http://fgnass.github.io/spin.js/), even though you set it in options.")}this._addLayerListeners(f);this._addMapListeners(f);if(!this.options.separate&&!this.zoomControl){if(f.zoomControl){this.zoomControl=f.zoomControl}else{if(f.zoomsliderControl){this.zoomControl=f.zoomsliderControl}}}var e="leaflet-control-loading";var d;if(this.zoomControl&&!this.options.separate){d=this.zoomControl._container;e+=" leaflet-bar-part-bottom leaflet-bar-part last";c.DomUtil.addClass(this._getLastControlButton(),"leaflet-bar-part-bottom")}else{d=c.DomUtil.create("div","leaflet-control-zoom leaflet-bar")}this._indicator=c.DomUtil.create("a",e,d);if(this.options.spinjs){this._spinner=new Spinner(this.options.spin).spin();this._indicator.appendChild(this._spinner.el)}return d},onRemove:function(d){this._removeLayerListeners(d);this._removeMapListeners(d)},removeFrom:function(d){if(this.zoomControl&&!this.options.separate){this._container.removeChild(this._indicator);this._map=null;this.onRemove(d);return this}else{return c.Control.prototype.removeFrom.call(this,d)}},addLoader:function(e){this._dataLoaders[e]=true;if(this.options.delayIndicator&&!this.delayIndicatorTimeout){var d=this;this.delayIndicatorTimeout=setTimeout(function(){d.updateIndicator();d.delayIndicatorTimeout=null},this.options.delayIndicator)}else{this.updateIndicator()}},removeLoader:function(d){delete this._dataLoaders[d];this.updateIndicator();if(this.options.delayIndicator&&this.delayIndicatorTimeout&&!this.isLoading()){clearTimeout(this.delayIndicatorTimeout);this.delayIndicatorTimeout=null}},updateIndicator:function(){if(this.isLoading()){this._showIndicator()}else{this._hideIndicator()}},isLoading:function(){return this._countLoaders()>0},_countLoaders:function(){var e=0,d;for(d in this._dataLoaders){if(this._dataLoaders.hasOwnProperty(d)){e++}}return e},_showIndicator:function(){c.DomUtil.addClass(this._indicator,"is-loading");if(!this.options.separate){if(this.zoomControl instanceof c.Control.Zoom){c.DomUtil.removeClass(this._getLastControlButton(),"leaflet-bar-part-bottom")}else{if(typeof c.Control.Zoomslider==="function"&&this.zoomControl instanceof c.Control.Zoomslider){c.DomUtil.removeClass(this.zoomControl._ui.zoomOut,"leaflet-bar-part-bottom")}}}},_hideIndicator:function(){c.DomUtil.removeClass(this._indicator,"is-loading");if(!this.options.separate){if(this.zoomControl instanceof c.Control.Zoom){c.DomUtil.addClass(this._getLastControlButton(),"leaflet-bar-part-bottom")}else{if(typeof c.Control.Zoomslider==="function"&&this.zoomControl instanceof c.Control.Zoomslider){c.DomUtil.addClass(this.zoomControl._ui.zoomOut,"leaflet-bar-part-bottom")}}}},_getLastControlButton:function(){var d=this.zoomControl._container,e=d.children.length-1;while(e>0){var f=d.children[e];if(!(this._indicator===f||f.offsetWidth===0||f.offsetHeight===0)){break}e--}return d.children[e]},_handleLoading:function(d){this.addLoader(this.getEventId(d))},_handleLoad:function(d){this.removeLoader(this.getEventId(d))},getEventId:function(d){if(d.id){return d.id}else{if(d.layer){return d.layer._leaflet_id}}return d.target._leaflet_id},_layerAdd:function(f){if(!f.layer||!f.layer.on){return}try{f.layer.on({loading:this._handleLoading,load:this._handleLoad},this)}catch(d){a.warn("L.Control.Loading: Tried and failed to add  event handlers to layer",f.layer);a.warn("L.Control.Loading: Full details",d)}},_layerRemove:function(f){if(!f.layer||!f.layer.off){return}try{f.layer.off({loading:this._handleLoading,load:this._handleLoad},this)}catch(d){a.warn("L.Control.Loading: Tried and failed to remove event handlers from layer",f.layer);a.warn("L.Control.Loading: Full details",d)}},_addLayerListeners:function(d){d.eachLayer(function(e){if(!e.on){return}e.on({loading:this._handleLoading,load:this._handleLoad},this)},this);d.on("layeradd",this._layerAdd,this);d.on("layerremove",this._layerRemove,this)},_removeLayerListeners:function(d){d.eachLayer(function(e){if(!e.off){return}e.off({loading:this._handleLoading,load:this._handleLoad},this)},this);d.off("layeradd",this._layerAdd,this);d.off("layerremove",this._layerRemove,this)},_addMapListeners:function(d){d.on({baselayerchange:this._handleLoading,dataloading:this._handleLoading,dataload:this._handleLoad,layerremove:this._handleLoad},this)},_removeMapListeners:function(d){d.off({baselayerchange:this._handleLoading,dataloading:this._handleLoading,dataload:this._handleLoad,layerremove:this._handleLoad},this)}});c.Map.addInitHook(function(){if(this.options.loadingControl){this.loadingControl=new c.Control.Loading();this.addControl(this.loadingControl)}});c.Control.loading=function(d){return new c.Control.Loading(d)}}if(typeof define==="function"&&define.amd){define(["leaflet"],function(c){b(c)})}else{b(L)}})();
/*!
Bing.js
https://github.com/shramov/leaflet-plugins/blob/master/layer/tile/Bing.js
*/
L.BingLayer=L.TileLayer.extend({options:{subdomains:[0,1,2,3],type:"Aerial",attribution:"Bing",culture:""},initialize:function(key,options){L.Util.setOptions(this,options);this._key=key;this._url=null;this._providers=[];this.metaRequested=false},tile2quad:function(x,y,z){var quad="";for(var i=z;i>0;i--){var digit=0;var mask=1<<(i-1);if((x&mask)!==0){digit+=1}if((y&mask)!==0){digit+=2}quad=quad+digit}return quad},getTileUrl:function(p,z){var zoom=this._getZoomForUrl();var subdomains=this.options.subdomains,s=this.options.subdomains[Math.abs((p.x+p.y)%subdomains.length)];return this._url.replace("{subdomain}",s).replace("{quadkey}",this.tile2quad(p.x,p.y,zoom)).replace("{culture}",this.options.culture)},loadMetadata:function(){if(this.metaRequested){return}this.metaRequested=true;var _this=this;var cbid="_bing_metadata_"+L.Util.stamp(this);window[cbid]=function(meta){window[cbid]=undefined;var e=document.getElementById(cbid);e.parentNode.removeChild(e);if(meta.errorDetails){return}_this.initMetadata(meta)};var urlScheme=(document.location.protocol==="file:")?"http":document.location.protocol.slice(0,-1);var url=urlScheme+"://dev.virtualearth.net/REST/v1/Imagery/Metadata/"+this.options.type+"?include=ImageryProviders&jsonp="+cbid+"&key="+this._key+"&UriScheme="+urlScheme;var script=document.createElement("script");script.type="text/javascript";script.src=url;script.id=cbid;document.getElementsByTagName("head")[0].appendChild(script)},initMetadata:function(meta){var r=meta.resourceSets[0].resources[0];this.options.subdomains=r.imageUrlSubdomains;this._url=r.imageUrl;if(r.imageryProviders){for(var i=0;i<r.imageryProviders.length;i++){var p=r.imageryProviders[i];for(var j=0;j<p.coverageAreas.length;j++){var c=p.coverageAreas[j];var coverage={zoomMin:c.zoomMin,zoomMax:c.zoomMax,active:false};var bounds=new L.LatLngBounds(new L.LatLng(c.bbox[0]+0.01,c.bbox[1]+0.01),new L.LatLng(c.bbox[2]-0.01,c.bbox[3]-0.01));coverage.bounds=bounds;coverage.attrib=p.attribution;this._providers.push(coverage)}}}this._update()},_update:function(){if(this._url===null||!this._map){return}this._update_attribution();L.TileLayer.prototype._update.apply(this,[])},_update_attribution:function(){var bounds=this._map.getBounds();var zoom=this._map.getZoom();for(var i=0;i<this._providers.length;i++){var p=this._providers[i];if((zoom<=p.zoomMax&&zoom>=p.zoomMin)&&bounds.intersects(p.bounds)){if(!p.active&&this._map.attributionControl){this._map.attributionControl.addAttribution(p.attrib)}p.active=true}else{if(p.active&&this._map.attributionControl){this._map.attributionControl.removeAttribution(p.attrib)}p.active=false}}},onAdd:function(map){this.loadMetadata();L.TileLayer.prototype.onAdd.apply(this,[map])},onRemove:function(map){for(var i=0;i<this._providers.length;i++){var p=this._providers[i];if(p.active&&this._map.attributionControl){this._map.attributionControl.removeAttribution(p.attrib);p.active=false}}L.TileLayer.prototype.onRemove.apply(this,[map])}});L.bingLayer=function(key,options){return new L.BingLayer(key,options)};
/*!
* FitText.js 1.2
*
