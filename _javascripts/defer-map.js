...
var activitiesMarkersCounter = 0;
var nbhdOnOpen = [];
var nbhdOnClose = [];
$("#Bos_nbhdSelect").multipleSelect({
    ...
    onClose: function() {
        ...
        if ($(nbhdOnOpen).not(nbhdOnClose).length === 0 && $(nbhdOnClose).not(nbhdOnOpen).length === 0) {} else {
            $("#mapOverlay").show();
            map.spin(true);
            setTimeout(function() {
                ...
                if (nbhdSelectedValuesObjLenght === 0) {
                    ...
                    $("#nbhdZoomBtn").prop("disabled", false);
                    activitiesMarkersCounter = 0;
                    myIndivOpenSpaceLayer.clearLayers();
                    myActivitiesMarkerClusterGroup.clearLayers();
                    $("#Bos_openSpaceSelect").multipleSelect("uncheckAll");
                    $("#activitiesSelect").clearMenu();
                    $("#openSpaceDeselectBtn, #activitiesDeselectBtn, #openSpaceZoomBtn").prop("disabled", true);
                ...
                alert(activitiesMarkersCounter);
                if (activitiesMarkersCounter === 0) {
                    $("#activitiesToggleBtn").prop("disabled", true);
                    $("#activitiesSelect").multipleSelect("disable")
                } else {
                    $("#activitiesToggleBtn").prop("disabled", false);
                    $("#activitiesSelect").multipleSelect("enable")
                }
                $("#mapOverlay").fadeOut();
                map.spin(false)
            }, 10)
        }
        if ($("#activitiesToggleBtn").hasClass("hideActivities")) {
            $("#activitiesToggleBtn").removeClass("hideActivities").addClass("showActivities");
            $("#activitiesToggleBtn").children("i").removeClass("fa-eye").addClass("fa-eye-slash")
        }
    },
});
...
$("#map").on("click", ".selectNbhdLink", function(event) {
    ...
    if (nbhdSelectedValuesObjLenght !== 0) {
        ...
        if (openSpaceSelectedValuesObjLenght !== 0) {
            ...
            myIndivOpenSpaceLayer.clearLayers();
            $("#Bos_openSpaceSelect").multipleSelect("uncheckAll");
            $("#openSpaceDeselectBtn, #openSpaceZoomBtn").prop("disabled", true);
            /*! ... and no open space is selected. */
        } else {
            alert("... and no open space is selected.")
        }
...
$("#map").on("click", ".deselectNbhdLink", function(event) {
    ...
    event.preventDefault();
    var nbhdValue = $(".nbhdPopupTitle").text();
    nbhdSelectedValues = jQuery.grep(nbhdSelectedValues, function(value) {
        return value != nbhdValue
    });
    var nbhdSelectedValuesObjLenght = Object.keys(nbhdSelectedValues).length;
    myNbhdLayer.clearLayers();
    myOpenSpaceLayer.clearLayers();
    ...
    if (openSpaceSelectedValuesObjLenght === 0) {
        ...
        if (nbhdSelectedValuesObjLenght === 0) {
            ...
            activitiesMarkersCounter = 0;
            alert("... and there are one or more other neighborhoods selected.");
            $("#nbhdZoomBtn").prop("disabled", false);
            $("#Bos_nbhdSelect").multipleSelect("setSelects", nbhdSelectedValues);
            $.each(nbhdSelectedValues, function(i, value) {
                L.geoJson(Bos_nbhd, {
 ...
 *  |   ===============
 */
$("#activitiesSelect").multipleSelect({placeholder:"Filter activities...",single:true,width:"100%",filter:true,allSelected:"1 of 1 activity selected",onClick:function(){var nbhdSelectedValues=$("#Bos_nbhdSelect").multipleSelect("getSelects");var nbhdSelectedValuesObjLenght=Object.keys(nbhdSelectedValues).length;var openSpaceSelectedValues=$("#Bos_openSpaceSelect").multipleSelect("getSelects");var openSpaceSelectedValuesObjLenght=Object.keys(openSpaceSelectedValues).length;myActivitiesMarkerClusterGroup.clearLayers();$("#mapOverlay").show();map.spin(true);setTimeout(function(){var activitiesSelectedValues=$("#activitiesSelect").multipleSelect("getSelects");var activitiesSelectedValuesObjLenght=Object.keys(activitiesSelectedValues).length;
/*! an activity is being selected, no neighborhoods are selected, and no open space is selected */
if(openSpaceSelectedValuesObjLenght===0){if(nbhdSelectedValuesObjLenght===0){alert("an activity is being selected, no neighborhoods are selected, and no open space is selected");var selectedActivityMarkers=L.geoJson(Bos_recActivities,{filter:function(feature,layer){if(feature.properties.Category==activitiesSelectedValues){return true}if(feature.properties.Category_2==activitiesSelectedValues){return true}if(feature.properties.Category_3==activitiesSelectedValues){return true}else{return false}},onEachFeature:$activitiesOnEachFeaturePopup,pointToLayer:$activitiesPointToLayer});myActivitiesMarkerClusterGroup.addLayer(selectedActivityMarkers);map.addLayer(myActivitiesMarkerClusterGroup);
/*! an activity is being selected, one or more neighborhoods are selected, and no open space is selected */
}else{alert("an activity is being selected, one or more neighborhoods are selected, and no open space is selected");$.each(nbhdSelectedValues,function(i,value){var selectedActivityMarkers=L.geoJson(Bos_recActivities,{filter:function(feature,layer){if((feature.properties.NBHD===value)&&(feature.properties.Category==activitiesSelectedValues)){return true}if((feature.properties.NBHD===value)&&(feature.properties.Category_2==activitiesSelectedValues)){return true}if((feature.properties.NBHD===value)&&(feature.properties.Category_3==activitiesSelectedValues)){return true}else{return false}},onEachFeature:$activitiesOnEachFeaturePopup,pointToLayer:$activitiesPointToLayer});myActivitiesMarkerClusterGroup.addLayer(selectedActivityMarkers);map.addLayer(myActivitiesMarkerClusterGroup)})}
