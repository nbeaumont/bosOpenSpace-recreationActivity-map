...
var nbhdOnOpen = [];
var nbhdOnClose = [];
$( "#Bos_nbhdSelect" ).selectmenu({
  
  
  open: function( event, ui ) {
        nbhdOnOpen = [];
        var nbhdSelectedValues= [];
$("#Bos_nbhdSelect :selected").each(function(i, selected){ 
  nbhdSelectedValues[i] = $(selected).text(); 
});
...
/*! ... and no open space is selected. */
}else{alert("... and no open space is selected.")}}myOpenSpaceLayer.bringToBack();myNbhdLayer.bringToBack();myBos_openSpace.bringToBack();myBos_nbhd.bringToBack();if(map.hasLayer(farmersMarkets)){farmersMarkets.addTo(myActivitiesMarkerClusterGroup)}if(map.hasLayer(publicArt)){publicArt.addTo(myActivitiesMarkerClusterGroup)}if(map.hasLayer(bikeTrails)){bikeTrails.bringToFront()}alert(activitiesMarkersCounter);if(activitiesMarkersCounter===0){$("#activitiesToggleBtn").prop("disabled",true);$("#activitiesSelect").multipleSelect("disable")}else{$("#activitiesToggleBtn").prop("disabled",false);$("#activitiesSelect").multipleSelect("enable")}if($("#activitiesToggleBtn").hasClass("hideActivities")){$("#activitiesToggleBtn").removeClass("hideActivities").addClass("showActivities");$("#activitiesToggleBtn").children("i").removeClass("fa-eye").addClass("fa-eye-slash")}$("#mapOverlay").fadeOut();map.spin(false);map.closePopup()});
/*!
 *  |   |   deselect
 *  |   |   --------
 */
